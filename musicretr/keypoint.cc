...
void writetext(unsigned int bits[], unsigned int nbits, char * fn) {
    FILE * f = fopen(fn, "wb");
    if (!f) {
        printf("Error: Can't open %s for writing.\n", fn);
        exit(1);
    ...
    std::string tmp;
    for (unsigned int i = 0; i < nbits; i++) {
        tmp = std::to_string(bits[i]) + "\n";
        fwrite(tmp.c_str(), tmp.length(), 1, f);
    }
...
void writetextbin(unsigned int bits[], unsigned int nbits, char * fn) {
    FILE * f = fopen(fn, "wb");
    if (!f) {
        printf("Error: Can't open %s for writing.\n", fn);
        exit(1);

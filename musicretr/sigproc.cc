...
float * wavread(const char * fn, unsigned int * nsamples, unsigned int * freq) {
	...
	if (header.subchunk2id != 0x61746164) {
        printf("Incorrect data chunk id(0x61746164), id=%x.\n", header.subchunk2id);
        return NULL;
	}
...
float ** wdft(float * samples, unsigned int nsamples, unsigned int * nframes) {
	...
	*nframes = (nsamples - FRAMESIZE) / OVERLAPSAMPLES;
	...
	float ** frames = (float **) malloc(sizeof(float *) * *nframes);
        ...
        printf("nframes: %d, nsample %d, frame/sample %d\n", *nframes, nsamples,
               nsamples/(*nframes));
...
unsigned int * wav2bits(vector<Filter> & filters, float * samples,
    ...
    if (nsamples > FRAMESIZE) {
        frames = wdft(dsamples, nsamples, &nframes);
    } // else bad wav file
	free(dsamples);
        // caculate nframes - KEYWIDTH / 2 in computebits
    if(frames == NULL || nframes < KEYWIDTH / 2) {
        *nbits = 0;
        return NULL;
    }
